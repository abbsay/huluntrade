...
export const Route = createFileRoute('/product/$productId')({
  // 1. USE LOADER: Fetch data before route loads (crucial for SSR and UX)
  loader: async ({ params }) => {
  ...
  // 2. USE LOADER DATA FOR HEAD: No double-fetching
  head: ({ loaderData }) => {
    const { product } = loaderData;
    const title = `${product.name} — Hulun Sweets`;
    const description = `Discover ${product.name} by Hulun Sweets. Fluffy, chewy, and cute candy made to bring joy and sweet smiles!`;
    return {
      meta: [
        { title },
  ...
  notFoundComponent: () => {
    return (
      <main className="min-h-[80vh] flex items-center justify-center bg-cream p-4">
        <motion.div
          className="max-w-md w-full text-center bg-white rounded-3xl p-10 shadow-xl border border-choco/5"
        ...
        >
          ...
          <h1 className="text-2xl font-black font-display text-choco mb-4">Product Not Found</h1>
          <Link
            to="/products"
            className="inline-block px-6 py-3 bg-strawberry text-white font-bold font-sans rounded-full shadow-md hover:scale-105 transition-transform"
          >
            Back to Products
          </Link>
        </motion.div>
...
function ProductDetail() {
  // Consume data from loader instead of fetching in component
    ...
    <main className={`min-h-[90vh] ${theme.bg} pt-32 md:pt-40 pb-32 relative overflow-hidden`}>
      ...
      <div className="max-w-[1240px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <div className={`relative w-full lg:w-1/2 p-10 md:p-16 flex items-center justify-center aspect-square lg:aspect-auto ${theme.imgBg}`}>
            ...
            <motion.div
              className="absolute top-6 right-6 md:top-10 md:right-10 bg-white p-3.5 rounded-2xl shadow-lg border border-choco/5 z-20 will-change-transform"
              aria-hidden="true"
              animate={{ y: [0, -8, 0] }}
