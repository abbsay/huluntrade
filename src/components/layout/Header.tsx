...
function switchLocaleUrl(targetLang: string) {
  if (typeof window === 'undefined') return;
  const pathname = window.location.pathname;
  const segments = pathname.split('/').filter(Boolean);
  let rawPath = pathname;
  if (segments.length > 0 && (SUPPORTED_LANGS as readonly string[]).includes(segments[0])) {
    rawPath = '/' + segments.slice(1).join('/');
    if (rawPath === '') rawPath = '/';
  }
  const targetUrl = targetLang === 'en'
    ? rawPath
    : (rawPath === '/' ? `/${targetLang}` : `/${targetLang}${rawPath}`);

  window.location.href = targetUrl;
}

function LangSwitcher({ variant = 'desktop', currentLang = 'en' }: { variant?: 'mobile' | 'desktop'; currentLang?: string }) {
  ...
  const activeOption = LANG_OPTIONS.find(o => o.code === currentLang) || LANG_OPTIONS[0];
    ...
    <div className="relative" ref={rootRef}>
      {variant === 'desktop' ? (
        ...
        >
          ...
          <span className={`text-[13px] font-medium tracking-tight uppercase transition-colors duration-300 ${isOpen ? 'text-choco' : 'text-mocha group-hover:text-choco'}`}>
            {activeOption.code}
          </span>
        </motion.button>
      ) : (
        ...
        >
          <img src="/images/categories/minimal_toy.svg" className="w-6 h-6 drop-shadow-sm object-contain" alt="Language" />
          <span className="uppercase tracking-wider">{activeOption.code}</span>
        </motion.button>
      )}
...
export function Header({ currentLang = 'en' }: { currentLang?: string }) {
      ...
      >
        <div className="max-w-[1400px] mx-auto px-4 md:px-6 xl:px-8">
          <div className="flex items-center justify-between h-[6.5rem]">
            ...
            {/* Logo */}
            <a href={homePath} onClick={closeMenu} className="flex-shrink-0 relative z-20">
              <Magnetic>
                <motion.img
                  src="/logo.png"
            ...
            <div className="hidden lg:flex items-center gap-4">
              <motion.a
                ...
                transition={{ type: 'spring', stiffness: 400, damping: 10 }}
              >
                <img src="/images/categories/minimal_jelly.svg" className="w-7 h-7 drop-shadow-sm object-contain" alt="Phone" />
              ...
              <a href={contactPath}>
                <motion.div
                  ...
                  whileTap={{ scale: 0.94 }}
                >
                  <img src="/images/categories/minimal_hard_candy.svg" className="w-6 h-6 drop-shadow-sm object-contain" alt="Catalogue" /> {t('nav.catalogue', 'Catalogue')}
                </motion.div>
              </a>

              <div className="h-6 w-px bg-mocha/20 mx-1" />
              <LangSwitcher variant="desktop" currentLang={currentLang} />
            </div>

            {/* Mobile Right Icons */}
            <div className="flex lg:hidden items-center gap-3 relative z-20">
              <LangSwitcher variant="mobile" currentLang={currentLang} />
                ...
                >
                  <div className="grid grid-cols-1 gap-4 text-center">
                    <motion.a
                      ...
                      whileTap={{ scale: 0.95 }}
                    >
                      <img src="/images/categories/minimal_jelly.svg" className="w-8 h-8 object-contain drop-shadow-sm" alt="Phone" />
                      {PHONE_DISPLAY}
                    </motion.a>
                    ...
                    <a href={contactPath} onClick={closeMenu}>
                      <motion.div
                        ...
                        whileTap={{ scale: 0.95 }}
                      >
                        <img src="/images/categories/minimal_hard_candy.svg" className="w-8 h-8 object-contain drop-shadow-sm" alt="Catalogue" />
