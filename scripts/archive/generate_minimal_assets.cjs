...
const publicDir = path.join(__dirname, 'madasweet-clone', 'public', 'images', 'categories');
...
// 2. Juicy Jelly M1 (Sleek Orange Amber/Gem Cube)
...
const toySVG = `<svg width="400" height="400" viewBox="0 0 400 400" xmlns="http://www.w3.org/2000/svg">
  ...
  <!-- Smooth Torus using evenodd path -->
...
console.log("💎 Abstract 3D Glassmorphic SVGs generated!");

// 5. Update index.tsx & HeroSlider.tsx & products.tsx references
function replaceImages(filePath) {
  if (!fs.existsSync(filePath)) return;
  let code = fs.readFileSync(filePath, 'utf8');
  code = code.replace(/\/images\/categories\/other\.png/g, '/images/categories/minimal_marshmallow.svg');
  code = code.replace(/\/images\/categories\/jelly\.png/g, '/images/categories/minimal_jelly.svg');
  code = code.replace(/\/images\/categories\/hard_candy\.png/g, '/images/categories/minimal_hard_candy.svg');
  // Need to distinguish the second "other.png" which was for candy toys
  // In `CATEGORIES` array it's usually the 4th object.
  code = code.replace(/\{ id: 'candy_toy'[\s\S]*?minimal_marshmallow\.svg'/g, (match) => {
    return match.replace('minimal_marshmallow.svg', 'minimal_toy.svg');
  });
  
  fs.writeFileSync(filePath, code);
}

replaceImages(path.join(__dirname, 'madasweet-clone', 'src', 'routes', 'index.tsx'));
replaceImages(path.join(__dirname, 'madasweet-clone', 'src', 'components', 'HeroSlider.tsx'));
replaceImages(path.join(__dirname, 'madasweet-clone', 'src', 'routes', 'products.tsx'));

console.log("Swapped density photos with premium abstract geometry.");
